...
const STATEMENT_DATA = [
    ...
    },
    {
        segments: [
            ...
            { text: " is often more important than knowing when to add it." }
        ]
    }
];

const FancyBullet = ({ isHovered, baseStatementDelay, index }) => {
...
const StatementCard = ({ statement, index, isHoverReady }) => {
    ...
    return (
        <motion.div 
            ...
            onAnimationStart={() => audio.playStatementReveal(index)}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.8, delay: baseStatementDelay, ease: [0.25, 1, 0.5, 1] }}
        >
            <FancyBullet isHovered={isHovered} baseStatementDelay={baseStatementDelay} index={index} />
            ...
            >
                <span className="lg:w-[95%] inline-block">
