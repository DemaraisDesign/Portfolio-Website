...
const IMAGES = {
  ...
  headshotPlaceholder: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&auto=format&fit=crop&q=60",
  experimentsBg: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=2670&auto=format&fit=crop"
};

// --- COMPONENT DEFINITIONS ---

const DisciplineModule = ({ title, img, color, darkColor, Icon, speed = 0.78, link, ...props }) => {
      ...
      <div className="relative aspect-[3/4] w-full">
        ...
        <div className="absolute inset-0 w-full h-full rounded-theme-sm border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.1)] overflow-hidden transform-gpu isolation-isolate z-10">
          <img
            src={img}
            alt={title}
            className="object-cover w-full h-full rounded-theme-sm"
          />
          <div
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-12 md:gap-16">
          <div className="w-full md:flex-1 order-2 md:order-1">
            <div>
              ...
              <motion.div className="space-y-6 text-brand-ink-body text-lg leading-relaxed mt-12" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-10%" }}>
                <motion.p variants={itemVariants}>
                  I started my career in Chicago's theater scene before heading to California for my MFA. There, I co-founded Coeurage Theatre Company in Los Angeles. We had a radical mission: every performance would be pay-what-you-want, so anyone could experience live theater regardless of their financial situation. We were in one of the most expensive cities in the world, competing for the same awards as theaters with massive budgets with relatively very little to work with. I served several roles, from performer to Resident Sound Designer to director, and eventually to artistic director.
                </motion.p>
                <motion.p variants={itemVariants}>
                  The constraints eventually became our creative crucible. Tiny budgets and high artistic ambitions forced us to use only what was essential—putting our resources toward what truly served the story and nothing more. Our artistic success attracted extraordinary artists who worked for far less than they were used to, and together we embraced the belief that limitation can breed innovation. We grew to become one of the most lauded companies in the city, and were recognized both locally and nationally. Stage & Cinema would eventually call me "the best director of his age in Los Angeles," but more importantly, those years forged the principles that would guide my work across every medium I've touched since.
                </motion.p>
                <motion.p variants={itemVariants}>
                  When COVID shut down live theater, a passing interest in digital design became something more urgent. I enrolled in a UX course and landed my first position on Morgan Stanley's award-winning Creative Services team through Oliver Agency. Suddenly I was working with screens instead of stages, and I realized something: the same philosophy that made me effective in theater applied everywhere. Whether I'm working with sound, screens, or stages, I'm always pursuing the same goals.
                </motion.p>
                <motion.p variants={itemVariants}>
                  <strong className="font-bold text-brand-ink">Working with sound requires knowing when to support, when to take control, and when to shut up.</strong> I could create the most elaborate soundscape—but if it's not serving the material, I've failed. And when working with live performers, I design for them as well as the audience. Sometimes that means practical choices to make their jobs easier. Sometimes that means designing to help them achieve an emotional response. This means paying close attention to their emerging artistic choices and staying open to shifting my own instincts is key.
                </motion.p>
                <motion.p variants={itemVariants}>
                  <strong className="font-bold text-brand-ink">Working with screens is about making the complex feel intuitive.</strong> It's taking dense information or systems and stripping away what isn't needed, combining what can be combined, arranging what remains so it feels second nature to people. UX research forces humility by nature—listening to what users say and what they don't say, searching for clues and connections to determine their needs, and staying open to changing your work. I can never fall into the trap of thinking artistic vision trumps the users I'm serving.
                </motion.p>
                <motion.p variants={itemVariants}>
                  <strong className="font-bold text-brand-ink">Working with stages is a fundamentally collaborative experience.</strong> I believe deeply that anyone can have a good idea, and that ideas are precious even when they're not used. Creating an atmosphere where creativity is invited from everyone, whether performer, stage manager, lighting designer, or anyone else in the room, gives a feeling of collective ownership and pride. It's also just good practice. Some of my most powerful creative choices evolved from other people's ideas or combinations of multiple perspectives.
                </motion.p>
                <motion.p variants={itemVariants}>
                  These aren't separate careers. They're different expressions of the same creative philosophy, shaped by those early years when necessity taught me to be efficient, collaboration taught me to be open, and ambition taught me never to let constraints define what's possible.
                </motion.p>
              </motion.div>
            </div>
          </div>
    ...
    <div ref={ref} className="relative w-full max-w-[1400px] mx-auto cursor-pointer focus-within:outline-none pt-16 lg:pt-0" onClick={() => navigate(`/work/${project.id}`)}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div className={`relative ${index % 2 === 1 ? 'lg:order-2' : ''}`} style={{ clipPath: "inset(0 round 7px)", WebkitClipPath: "inset(0 round 7px)" }}>
          <div className="aspect-video relative bg-brand-dark overflow-hidden rounded-theme-sm">
            <motion.div className="absolute inset-0 w-full h-full" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 1.2, ease: [0.25, 1, 0.5, 1] }}>
              <motion.img style={{ y, scale: 1.15 }} src={project.img} alt={`${project.title} Project Image`} className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
            </motion.div>
          </div>
        </div>
...
const Works = () => {
  ...
  return (
    <section ref={ref} id="works" className="bg-black text-white py-24 lg:py-32 overflow-hidden relative">
      ...
      <div className="flex flex-col space-y-16 lg:space-y-32 px-9 md:px-12 lg:px-24 relative z-10 divide-y divide-white/20 lg:divide-y-0">
      ...
      <div className="w-full max-w-[1400px] mx-auto px-9 md:px-12 lg:px-24 flex flex-col-reverse md:flex-row items-start justify-between mb-16 gap-12">
        <div>
          <h2
            ...
            className="text-5xl md:text-7xl font-extrabold text-brand-ink font-outfit uppercase tracking-tight mb-6 cursor-pointer hover:text-brand-orange transition-colors duration-300"
          >
            Explorations
          </h2>
        ...
        <div ref={heroRef} className="relative h-screen w-full overflow-hidden">
          ...
          <div className="absolute h-full top-0 left-0 w-full pointer-events-none z-10 flex items-center justify-start pl-6 md:pl-20">
            ...
            >
              ...
              <p className="font-outfit font-light text-brand-ink/80 m-0 text-[clamp(1.5rem,3vmin,2.5rem)] max-w-2xl">
